...
\subsubsection{Optimisation of Asset Allocation Policies}


The optimisation of \(\theta\) is the core of our analysis, as it allows us to find the asset allocation strategy that maximises our chosen risk measure of the resulting wealth trajectories. \textcite{Makinen2024} use an adjoint method to compute the gradient of the risk measure with respect to the policy parameters, which allows for efficient optimisation even in high-dimensional parameter spaces. This is a powerful and efficient approach, but it is also quite complex to implement and limits the potential range of cost functions that can be used as the risk measure to those that only depend on the terminal wealth distribution. For this reason, we have implemented a more straightforward approach that relies on automatic differentiation using the PyTorch Python library \parencite{pytorch}. This allows us to compute the gradient of the risk measure with respect to the policy parameters without having to derive and implement the adjoint equations, and it also allows us to use a wider range of risk measures that can depend on the entire wealth trajectory rather than just the terminal wealth distribution. The trade-off is that this approach is likely to be less computationally efficient than the adjoint method, particularly as the dimensionality of the policy parameter space increases, but we believe that the increased flexibility in terms of risk measures and ease of implementation makes it a worthwhile trade-off for our analysis.

The construction of our simulation framework has been carefully designed to be fully differentiable (using PyTorch's automatic differentiation capabilities) so that we can compute the gradient of our chosen risk measure with respect to the policy parameters \(\theta\) and use this gradient to perform optimisation using a gradient-based algorithm such as stochastic gradient descent or Adam \parencite{adam}. This allows us to efficiently search for the optimal policy parameters that maximise our chosen risk measure, even in high-dimensional parameter spaces.

To constrain the policy parameters \(\theta\) to satisfy the allocation constraints from Equation \ref{allocation_constraints} (i.e., that the allocations must be non-negative and sum to at most one), we project the parameters onto the feasible set after each gradient update using the simplex projection algorithm of \textcite{duchi2008efficient}. This efficient \(\mathcal{O}(n \log n)\) algorithm finds the Euclidean projection onto the constraint set, ensuring that policy parameters remain feasible while staying as close as possible to the unconstrained gradient update. The projection preserves the optimality conditions for constrained optimisation, guaranteeing convergence to a local optimum on the constraint boundary under standard regularity conditions. A detailed description of the algorithm is provided in Appendix \ref{appendix:simplex_projection}.


\section{Methods and Data for Simulating and Bootstrapping Asset Returns}
...
\subsection{Geometric Brownian Motion (GBM) Model for Synthetic Return Generation}
...
\subsection{Block Bootstrap Methodology}
...
\subsubsection{Data Sources and Asset Class Construction}
...
\section{Optimal Retirement Asset Allocation Strategies for New Zealand Retirees}
...
\subsection{Investor Preferences and Risk Measures}
...
\subsubsection{Heuristics of Risk and Success}

The relative sucess of a strategy is informed by the conservative nature of the hypothesised retirees, who are primarily concerned with the risk of running out of money. To capture this, we focus on two key metrics: the bankruptcy density and the impoverishment density. The bankruptcy density measures how frequently wealth hits zero across the entire simulation ensemble and time horizon, while the impoverishment density counts occurrences where ex-post real consumption falls below the inflation-adjusted consumption floor. These metrics provide a more direct measure of the risks that our retirees are concerned about.

Let $W_{i,t}$ and $C_{i,t}$ denote wealth and consumption for simulation $i\in\{1,\dots,N_{\mathrm{sims}}\}$ at time $t\in\{1,\dots,T\}$. Define the binary bankruptcy indicator
\begin{equation}\label{eq:bankruptcy_indicator}
B_{i,t} \;=\; \begin{cases} 1 & \text{if } W_{i,t} = 0, \\ 0 & \text{otherwise.} \end{cases}
\end{equation}

The \emph{bankruptcy density} measures how frequently wealth hits zero across the entire simulation ensemble and time horizon:
\begin{equation}\label{eq:bankruptcy_density}
\mathrm{Bankruptcy\ Density} \,=\, \frac{1}{N_{\mathrm{sims}}\,T} \sum_{i=1}^{N_{\mathrm{sims}}}\sum_{t=1}^{T} B_{i,t}.
\end{equation}

The \emph{impoverishment density} counts occurrences where ex-post real consumption falls below the inflation-adjusted consumption floor $F_{i,t}$ (see Equation \ref{real_consumption_floor}). Using the indicator
\begin{equation}\label{eq:impoverishment_indicator}
I_{i,t} \;=\; \begin{cases} 1 & \text{if } C_{i,t} < F_{i,t}, \\ 0 & \text{otherwise,} \end{cases}
\end{equation}
we define
\begin{equation}\label{eq:impoverishment_density}
\mathrm{Impoverishment\ Density} \,=\, \frac{1}{N_{\mathrm{sims}}\,T} \sum_{i=1}^{N_{\mathrm{sims}}}\sum_{t=1}^{T} I_{i,t}.
\end{equation}
...
\subsubsection{Cost Functions for Optimisation}

The heuristics described above provide interpretable metrics for evaluating asset allocation strategies, but they are not differentiable with respect to policy parameters $\theta$ and are thus unsuitable for gradient-based optimisation. We therefore construct a differentiable cost function that serves as a proxy for these risk measures and encourages the optimiser to find policies aligned with retiree preferences.
...
\subsection{Asset Allocation in One Dimension: Time-Based Glide-Paths}
