...
We begin with the simplest case: a policy with zero decision nodes, which we refer to as a fixed allocation strategy. A fixed allocation strategy maintains constant weights across all assets throughout the retirement horizon, regardless of time elapsed or current wealth. Such a policy can be represented by a single parameter vector $\theta$ that specifies the allocation proportions and does not vary with time or wealth. Fixed allocation strategies serve three important roles in our analysis. First, they provide a transparent baseline against which to measure the performance gains of more sophisticated, dynamic strategies. Second, they isolate the effects of asset class selection and return dynamics from any adaptive rebalancing benefits. Third, because the optimisation landscape is considerably simpler in the zero-dimensional case, we can achieve reliable convergence and develop confidence in our optimisation procedure before scaling to higher-dimensional policies. 

Because we are not simulating the accumulation period of our retirees, we do not have a specific initial wealth level to use in our simulations. Instead, we choose to treat initial wealth as an experimental parameter and we run our optimisation procedure over a range of fixed initial wealth levels. This allows us to evaluate how the optimal fixed allocation strategy varies with the retiree's starting wealth, and to assess the robustness of the resulting strategies across different wealth levels. We varied the initial wealth levels from \$500,000 to \$1,000,000 in increments of \$50,000. The resulting allocations are shown in Figure \ref{fig:fixed_allocation_results}. 

Studying how the optimal allocation varies with initial wealth provides some interesting insights into the dynamics of the problem and the nature of the datasets. The 1870--present bootstrapped sample produces fixed 100\% equity allocations for all initial wealth levels, while the 1950--present bootstrapped sample and the GBM model produce allocations that vary with initial wealth. The reason for the dominance of the all-equity allocation in the 1870--present bootstrapped sample is likely due to the presence of various historic periods of high domestic inflation, for which the unhedged international equity exposure in our asset class construction provides a natural hedge. We will discuss this further in a later section, but for now we note that the presence of these historic periods of high inflation in the 1870--present bootstrapped sample is likely to be a key driver of the optimal allocation to equities in this case. The 1950--present bootstrapped sample and the GBM model, on the other hand, do not contain these historic periods of high inflation, and therefore the optimal allocation to equities is lower for these datasets.

Both the 1950--present bootstrapped sample and the GBM model show a clear trend of decreasing equity allocation with increasing initial wealth, with low-wealth retirees being optimally allocated 100\% to equities. This is an interesting dynamic, and it might be counter-intuitive at first glance. One might expect that retirees with higher initial wealth levels would be able to take on more risk and therefore allocate more heavily to equities, while lower-wealth retirees would need to be more conservative in their allocations. However, the optimisation procedure is designed to minimise the risk of running out of money, and for low-wealth retirees, the major risk to their retirement security is not in fact market risk but rather the fact that consumption already exceeds the capacity of their savings and they are therefore at a high risk of running out of money irrespective of market dynamics. We have come to term this the ``hail-mary zone'' of wealth where the optimal strategy is to maximise expected returns at all costs and thus to allocate entirely to the riskiest assets in the hope that favourable market returns will allow them to achieve a sufficient level of wealth to accommodate their desired spending targets. On the other hand, for higher-wealth retirees the risk of exceeding the consumptive capacity of their savings is lower, and so market risk becomes a more significant factor in the optimisation. In this case, the optimal strategy is to diversify into safer asset classes to reduce market risk and protect the consumptive capacity of their wealth in the face of adverse market conditions. In other words, when the investor's wealth and consumption requirements place them in the hail-mary zone, the strategy becomes subject to \textit{budgetary dominance} and the optimal allocation aims to maximise expected asset returns in the hope of increasing their consumptive capacity. Whereas, when the investor's wealth and spending requirements place them outside of the hail-mary zone, the strategy becomes \textit{market dominated} and the optimal allocation increasingly aims to protect the consumptive capacity of their savings in the event of adverse market conditions.

We speculate that the point at which the optimal allocation switches from the ``hail-mary'' approach to one that minimises market risk is likely an important threshold in the optimisation problem. Another noteworthy effect of this interplay of risks can be seen in the terminal wealth statistics. In both the GBM model and the 1950--present bootstrapped sample, mean terminal wealth actually decreases slightly as initial wealth increases. In the GBM model, this occurs for initial wealth approximately \$700k, and in the 1950--present bootstrap, for initial wealth levels between approximately \$600k and \$750k. We think that this shows the optimal strategy essentially trading long-term expected returns for market risk reduction, resulting in simultaneously lower levels of terminal wealth and lower likelihoods of running out of money. We also think that another reason that the 1870--present bootstrapped sample produces a 100\% equity allocation for all initial wealth levels is likely due to the fact that the presence of historic periods of high inflation in this dataset means that the retiree's expected spending is systematically higher across that ensemble of return paths. Therefore all scenarios tested in that dataset are budgetarily dominated ``hail-mary'' scenarios.

Another interesting difference between the datasets is the presence of systematically higher allocations to bonds in the GBM model relative to the 1950--present bootstrapped sample. This is similar to findings by \textcite{anarkulova2025beyond}, who found that IID return assumptions (as in the GBM model) which ignore time-series effects tend to over-allocate to bonds relative to bootstrapped return assumptions. While both the GBM and 1950--present allocations have comparable allocations to equities, the GBM model has higher allocations to bonds and lower allocations to cash, whereas the 1950--present bootstrapped sample almost never allocates to bonds. We will investigate this further in a later section.


\begin{figure}[ht!]
    \centering
    \includegraphics[width=1\textwidth]{../Code/Analysis/Plots/Fixed_Policy_Analysis.png}
    \caption{Optimal fixed allocation strategies for different initial wealth levels and return sampling methodologies. Figures (a, b, c) show the optimal allocations for the GBM model, the 1870--present bootstrapped sample, and the 1950--present bootstrapped sample  respectively. The x-axis shows the initial wealth level, and the y-axis shows the allocation to each asset class. Figures (d, e, f) show the corresponding bankruptcy densities for the optimal fixed allocation strategies under each return sampling methodology. Figures (g, h, i) show the bankruptcy probabilities for the optimal fixed allocation strategies under each return sampling methodology. (j, k, l) show the corresponding expected final wealth distributions for the optimal fixed allocation strategies under each return sampling methodology.}
    \label{fig:fixed_allocation_results}
\end{figure}
...
\begin{figure}
    ...
    \caption{Evolution of the optimal glide-path policy matrix as we increase the number of time nodes and the initial wealth of the investor. Each plot shows the optimal allocation to cash and bonds interpolated over time (as it is done in the simulation), with the equities allocation (not shown) being the remainder. Node count increases from right to left, with the fixed policy shown on the far right of each plot.}
...
\subsection{Asset Allocation in One Dimension: Wealth-Based Glide-Paths}
