...
We now combine both state variables and optimise a two-dimensional control matrix, where allocations depend jointly on retirement time and current wealth. For each market condition generator, we estimate unified policies over distributed initial wealth buckets (\$500{,}000 to \$1{,}000{,}000 in \$50{,}000 increments), and sweep matrix resolutions from coarse to finer grids in both dimensions.
...
A further and more pointed finding concerns the relative performance of glidepaths that were optimised with full knowledge of the retiree's initial wealth. Even these idealised, wealth-calibrated glidepath strategies are matched or outperformed by the unified wealth-responsive policies (Figure \ref{fig:wealth_based_vs_known_wealth_glidepath_bankruptcy_density}). Recall that the wealth-responsive strategies were optimised jointly over the entire wealth distribution, with no access to the individual's starting position; the glidepath baselines had this information and were calibrated specifically for each wealth level. The fact that the glidepath strategies only outperformed the unified wealth-responsive policies for the highest initial wealth levels, and that even that advantage was limited to a small number of configurations, demonstrates that real-time adaptation to current wealth state has an advantage over even the most aggressively tailored age-based schedule.

Within any age cohort, the distribution of wealth, income, spending needs, health, and risk preference is very wide. Two 65-year-olds who appear identical from the perspective of a target-date fund may occupy very different positions in the financial landscape: one may be in a hail-mary zone where the only viable response is to maximise expected returns, while the other may have sufficient wealth that inflation hedging and sequence-risk management are the dominant concerns. Prescribing the same allocation trajectory to both, on the basis of their shared age, is a coarse approximation. Current wealth is a substantially more informative signal for determining the appropriate allocation; policies that condition on it can continuously distinguish between these two situations as they evolve over the retirement horizon, rather than committing to a predetermined schedule regardless of how circumstances unfold.

It is worth noting that the heterogeneity of optimal glidepath shapes across initial wealth levels (Figure \ref{fig:glide_path_evolution}) is, in a certain sense, itself encoding a wealth-responsive component. The glidepaths change shape precisely because the underlying economic situation of the retiree changes with wealth. In effect, the wealth-specific glidepaths are approximating a wealth-responsive policy through a time-based parameterisation. That the wealth-responsive strategies achieve similar or superior performance more directly, and without requiring prior knowledge of initial wealth, suggests that this approximation is unnecessary and that the underlying mechanism should be targeted directly.

There are genuine limitations to these conclusions. We have not modelled age-contingent healthcare costs, which may represent a meaningful expenditure shock at advanced ages and could provide a legitimate motivation for some age-dependent allocation adjustment. Our spending model is also calibrated to a representative New Zealand household rather than to individual-level heterogeneity, which may understate the variance of outcomes within age cohorts. Moreover, the advantages of the wealth-responsive policies are most evident at middle wealth levels; the differences narrow at the extremes of the wealth distribution, where budgetary dominance or substantial wealth buffer largely determine the outcome regardless of the allocation mechanism. Despite these caveats, the consistent pattern across all three market condition generators is that current wealth is a more powerful conditioning variable than elapsed time, and that policies conditioned on wealth outperform their time-based counterparts across most wealth levels and return environments.

For practical purposes, this suggests that the widespread use of target-date funds as a retirement allocation vehicle warrants scrutiny. While appear to offer a intuitively appealing solution to the problem of life-cycle investing, they are fundamentally limited by their reliance on age as the primary conditioning variable. Moreover, the declining-risk structure that is often implemented in these products is not supported by any of the optimal strategies we have observed, and the empirical literature on glidepath shape is inconclusive at best. It is hard not to conclude that the popularity of these products is driven more by their marketability than by evidence-based financial planning.
...
However, two important caveats apply. First, the adviser must be prepared to revisit and update the strategy as the client's circumstances evolve. A glidepath calibrated to a client's situation at age 65 may become inappropriate by age 75 if their wealth has been substantially depleted by adverse market returns or has grown beyond expectations. In the former case, the client may have moved into the hail-mary zone and require a more aggressive posture than the original schedule specifies; in the latter, a more conservative approach may be warranted. A static predetermined schedule, left unrevised, could therefore produce poor outcomes even from a well-calibrated starting point. Second, the glidepath should be understood as a rough schedule rather than a precise optimum: our results show that performance differences between reasonably calibrated time-varying strategies and simple fixed allocations are often modest, and that the primary determinant of outcomes is the overall level of equity exposure rather than the exact trajectory.

What may be more valuable than a glidepath recommendation per se is guidance around the concept of a client's \textit{safe wealth threshold}, that is, the wealth level above which projected spending is unlikely to exhaust the portfolio under plausible market outcomes, and below which more aggressive growth strategies become appropriate. This threshold is not fixed; it depends on the client's spending targets, remaining life expectancy, and the prevailing return environment. Providing advisers with a clear framework for assessing where their client sits relative to this threshold, and how the appropriate allocation changes as a result, would translate the core insight of the wealth-responsive strategies directly into advisory practice. Unlike a glidepath, this approach adapts naturally to changing circumstances without requiring a new predetermined schedule to be specified each time circumstances shift.
...
\begin{figure}[ht!]
    ...
    \caption{Autocorrelation function (ACF, left) and partial autocorrelation function (PACF, right) of the annual CPI inflation rate for the two block-bootstrapped generators. The grey shaded band indicates the approximate 95\% white-noise confidence interval ($\pm 1.96/\sqrt{T}$). The dotted blue line marks the IID reference of zero autocorrelation. Both bootstrapped series exhibit a substantial positive lag-1 ACF and a PACF that drops sharply after lag 1, consistent with an AR(1) inflation process. This persistence implies that a single bad inflation year predicts several further years of elevated inflation, precisely the regime under which nominal bonds suffer sustained real capital losses.}
...
\subsection{Projected Gradients Before Adam Updates}
